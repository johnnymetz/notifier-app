...
def test_create_verify_refresh_jwt(client, freezer, settings):
    pw = "pass"
    u = UserFactory(password=pw, is_active=False)

    create_url = reverse("jwt-create")
    r = client.post(create_url, {"username": u.email, "password": pw})
    assert r.status_code == status.HTTP_400_BAD_REQUEST
    r = client.post(create_url, {"email": u.email, "password": pw})
...
def test_client_login(client):
    ...
    is_logged_in = client.login(email=u.email, password=pw)
    assert is_logged_in
